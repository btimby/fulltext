...
def windowsUnicode(string):
    ...
    if sys.version_info[0] >= 3:  # Python 3
        ...
        return unicode(string, 'utf_16_le')  # NOQA
...
class Message(OleFile.OleFileIO):
    ...
    def attachments(self):
        try:
            ...
            for dir_ in self.listdir():
                if dir_[0].startswith('__attach') and \
                        dir_[0] not in attachmentDirs:
    ...
    def save(self, toJson=False, useFileName=False, raw=False):
        ...
        Setting toJson to true will output the message body as
        JSON-formatted text.  The body and attachments are stored in a folder.
        Setting useFileName to true will mean that the filename is used as
        the name of the folder; otherwise, the message's date
        ...
        if useFileName:
            ...
            if self.subject is None:
                ...
                subject = "".join(
                    i for i in self.subject if i not in r'\/:*?"<>|')
