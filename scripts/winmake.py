...
def uninstall():
    """Uninstall fulltext"""
    # Uninstalling fulltext on Windows seems to be tricky.
    # On "import fulltext" tests may import a fulltext version living in
    # C:\PythonXY\Lib\site-packages which is not what we want, so
    # we try both "pip uninstall fulltext" and manually remove stuff
    # from site-packages.
    clean()
    install_pip()
    here = os.getcwd()
...
def pydeps():
    """Install deps"""
    install_pip()
    req = os.path.realpath(os.path.join(HERE, '../requirements.txt'))
    sh("%s -m pip install -U -r %s" % (PYTHON, req))
